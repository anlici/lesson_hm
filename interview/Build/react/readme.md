...
- 设计理念：
  - vue：
    ...
    - vue 的核心是*响应式系统*，通过数据驱动视图的更新。我们更多关注于图层变化，去按需引入路由
...
- 合成事件机制(Synthetic Events)
  合成事件是React特有的事件处理系统：
- 默认情况下，React事件只支持*事件冒泡*
- 定义 ：React将原生DOM事件包装成合成事件对象，统一绑定到根节点(document)
- 工作原理 ：
  - **事件委托**：所有事件都委托到根节点，而不是直接绑定到DOM元素
  - 事件池：*缓存/复用事件对象*（减少频繁创建和销毁事件对象），性能优化
  - *事件分发*：根据**事件冒泡路径，找到对应组件并执行其事件处理函数**（如onClick）
### 主要区别
- 目的不同 ：
  - 水合：解决SSR与客户端渲染的衔接问题
...
- 硬件，
- 大模型，
  逻辑记忆，手动实现；agent 记忆模块，大模型上下文token 管理，

## fiber 渲染机制（高效，可中断）增调双树两阶段
- 早期往常：一次性渲染所有 组件，主线程阻塞
- 核心：增量渲染，优先级调度、双缓存树
- 点击事件可以中断，列表渲染（render）
- 新建树一次性被切换掉，旧树 （conmmit阶段）
它实现了*增量渲染*的能力，就是把渲染任务分割成多个小任务，配合优先级调度机制，让高优先级的任务（如用户输入）可以**中断低优先级**的任务（如大列表渲染），提升用户体验。

其次，它采用了**双缓存树的设计**。在内存中会同时存在两棵树：一棵是当前页面对应的树（current树），
另一棵是正在构建的树（workInProgress树）。这种机制让React可以在后台准备新的树，完成后再一次性替换到屏幕上，
避免了中间状态造成的视觉问题。

最后，整个更新过程分为两个阶段：**render阶段和commit阶段**。render阶段负责构建Fiber树和标记副作用，这个阶段是可以被中断的；而commit阶段负责把这些变更真正应用到DOM上，这个阶段必须一次性完成，不能中断。
这样的设计既保证了渲染的可中断性，又确保了DOM更新的原子性。
...
- react hooks：hooks 是 react 16.8 新增特性，区别于类组件
  可以在函数组件使用状态和其他react特性；
  打破函数组件只能渲染ui，实现使用状态和生命周期的特性
  一般以use 开头，没有类组件 this 的指向
  useContext 组件树传递上下文（context），避免通过逐层传递props 
  
  麻烦比如：主题色，通过上下文在多组件共享
- utils 是自己写的一些方法，用于解决一些问题。（防抖）
- 实现某些逻辑，减少重复代码，不依赖于react生命周期
## react18 新特性
- react18 异步渲染
- 提供canvas 渲染功能
