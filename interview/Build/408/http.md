...
## 常见状态码
- 400 客户端请求错误，可能参数格式，请求体
- 403 禁止访问，权限不够，服务器理解但是不给
- 301 永久重定向，下次串新的
- 302 临时
- 307 临时，保留请求体和方法
- 308 永久，保留请求体和方法
...
## 中间件处理错误
- 类型：网络：请求响应拦截；不同状态码匹配不同错误处理
- 业务逻辑：在意图分析，sql生成，prompt，大模型输出，格式校验，数据处理，返回结果
- 数据层：数据库传到前端，字段属性和名称 
- 比如：express 中间件next(err) 
####  koa  洋葱
- 中间件栈，按照顺序；next 暂停当前中间件，执行下一个中间件
- 执行完回溯，控制权会给上一个中间件
```js
